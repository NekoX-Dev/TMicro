...
public class FullTextIndexImpl extends PersistentResource implements FullTextIndex {
    ...
    public void writeObject(IOutputStream out) {
        out.writeObject(inverseIndex);
        out.writeObject(documents);
        out.writeObject(helper);
    }

    public void readObject(IInputStream in) {
        inverseIndex = (Index) in.readObject();
        documents = (Index) in.readObject();
        helper = (FullTextSearchHelper) in.readObject();
    }
    ...
    static class KeywordIterator extends Iterator {
    ...
    public Iterator getKeywords(String prefix) {
        return new KeywordIterator(inverseIndex.entryIterator(new Key(prefix),
                new Key(prefix + Btree.MAX_CHAR, false),
                Index.ASCENT_ORDER));
    }
    ...
    static class DocumentOccurrences extends Persistent {
        ...
        public void writeObject(IOutputStream out) {
            ...
            out.writeInt(nWordsInDocument);
            out.writeArrayOfByte(occurrences);
        }

        public void readObject(IInputStream in) {
            list = (InverseList) in.readObject();
            nWordsInDocument = in.readInt();
            occurrences = in.readArrayOfByte();
        }
    }
    ...
    static class Document extends Persistent {
        ...
        public void writeObject(IOutputStream out) {
            out.writeObject(obj);
            out.writeLink(occurrences);
        }

        public void readObject(IInputStream in) {
            obj = in.readObject();
            occurrences = in.readLink();
        }
    ...
    static class InverseList extends Btree {
        ...
        public void writeObject(IOutputStream out) {
            ...
            out.writeArrayOfInt(oids);
            out.writeLink(docs);
        }

        public void readObject(IInputStream in) {
            super.readObject(in);
            oids = in.readArrayOfInt();
            docs = in.readLink();
        }
